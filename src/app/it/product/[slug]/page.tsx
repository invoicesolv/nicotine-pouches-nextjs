...
async function getProduct(slug: string, packSize: string = '1pack', shippingFilter: string = 'fastest', sortFilter: string = 'low-high') {
  try {
    ...
    // If not found, try case-insensitive ilike search with a single query
    if (error || !product) {
      // Check if the first query failed due to a connection error (not just "not found")
      if (error && error.message && (error.message.includes('<!DOCTYPE') || error.message.includes('fetch') || error.message.includes('abort'))) {
        console.error('Supabase connection error:', error.message.substring(0, 100));
        return null;
      }
      ...
      if (!ilikeError && ilikeProducts && ilikeProducts.length > 0) {
        // Score matches: exact > starts with > contains
        product = ilikeProducts.reduce((best: any, p: any) => {
          const pLower = p.name.toLowerCase();
          const searchLower = baseSearchTerm.toLowerCase();
          const properLower = properCaseName.toLowerCase();

          let score = 0;
          if (pLower === properLower || pLower === searchLower) score = 100;
          else if (pLower.startsWith(searchLower) || pLower.startsWith(properLower)) score = 50;
          else score = 10;

          // Bonus for X- pattern match
          if (hasXPattern && /X-/i.test(p.name)) score += 20;
        ...
        if (product) delete product._matchScore;
      }

      // Fallback: fuzzy search by individual words (single query)
      if (!product) {
        ...
        if (words.length > 0) {
          ...
          if (!fuzzyError && fuzzyProducts && fuzzyProducts.length > 0) {
            product = fuzzyProducts.reduce((best: any, p: any) => {
              ...
              if (hasXPattern) {
                ...
                if (hasXInName && !bestHasX) return p;
                if (bestHasX && !hasXInName) return best;
              }
    ...
    const { data: mappings, error: mappingError } = await supabase()
      ...
      .select('vendor_product, it_vendor_id')
    ...
    if (mappings && mappings.length > 0) {
      ...
      const { data: vpData, error: vpError } = await supabase()
        ...
        .in('it_vendor_id', mappings.map((m: any) => m.it_vendor_id))
        .limit(20);
    ...
    vendorProducts?.forEach((vp: any) => {
      ...
      if (!storesMap.has(vendorKey)) {
        storesMap.set(vendorKey, {
          ...
          review_count: 0,
          shipping_info: 'Spedizione standard',
          shipping_cost: 0,
          free_shipping_threshold: 0,
          delivery_speed: null,
          vendorId: vp.it_vendor_id,
      ...
      // Build previous prices map (convert DB keys like price_1pack -> 1pack and apply currency conversion)
      let prevPrices: Record<string, string> | null = null;
      if (vp.previous_prices && vp.price_changed_at) {
        const raw: Record<string, any> = {};
        for (const [dbKey, val] of Object.entries(vp.previous_prices as Record<string, any>)) {
          const uiKey = dbKey.replace('price_', '');
      ...
      storesMap.get(vendorKey).variants.push({
        ...
        link: vp.url || '#',
          prices: validPrices,
          previous_prices: prevPrices,
          price_changed_at: vp.price_changed_at || null,
          in_stock: isInStock,
...
export default async function ProductPage({ params, searchParams }: ProductPageProps) {
  ...
  const [ratingResult, ...reviewResults] = await Promise.all([
    ...
    ...(vendorIds.length > 0 ? [
      supabase().from('it_vendors').select('it_vendor_id').in('it_vendor_id', vendorIds).eq('is_approved', true),
      supabase().from('it_vendors').select('it_vendor_id').in('it_vendor_id', vendorIds),
      supabase().from('it_vendors').select('id, name, logo_url').in('id', vendorIds),
      supabase().from('it_vendors').select('*').in('it_vendor_id', vendorIds)
        .order('review_date', { ascending: false, nullsFirst: false }).limit(10),
      supabase().from('it_vendors').select('*', { count: 'exact', head: true }).in('it_vendor_id', vendorIds)
    ] : [])
  ]);
  ...
  if (vendorIds.length > 0) {
    ...
    userReviews?.data?.forEach((r: any) => {
      vendorReviewCountMap.set(r.it_vendor_id, (vendorReviewCountMap.get(r.it_vendor_id) || 0) + 1);
    });
    ...
    reviewsResult?.data?.forEach((review: any) => {
      allTrustpilotReviews.push({
        ...review,
        vendor_name: vendorNameMap.get(review.it_vendor_id) || 'Unknown Vendor',
        vendor_logo: vendorLogoMap.get(review.it_vendor_id) || null
      });
    });
  }
  ...
  try {
    ...
    if (brandProducts && brandProducts.length > 0) {
      ...
      const [mappingsResult, trackingResult] = await Promise.all([
        supabase().from('it_vendor_product_mapping').select('product_id, de_it_vendor_id, vendor_product').in('product_id', productIds),
        supabase().from('it_vendor_products').select('product_id').in('product_id', productIds)
      ]);
      ...
      vpMappings.forEach((m: any) => {
        ...
        vendorSets.get(m.product_id)!.add(m.it_vendor_id);
      });
      ...
      if (vpMappings.length > 0) {
        const { data: vpPrices } = await supabase()
          .from('it_vendor_products')
          .select('name, it_vendor_id, price_1pack')
          .in('name', vpMappings.map((m: any) => m.vendor_product))
          .in('it_vendor_id', vpMappings.map((m: any) => m.it_vendor_id))
        ...
        if (vpPrices) {
          ...
          vpMappings.forEach((m: any) => {
            nameVendorToProduct.set(`${m.vendor_product}__${m.it_vendor_id}`, m.product_id);
          });

          vpPrices.forEach((vp: any) => {
            const productId = nameVendorToProduct.get(`${vp.name}__${vp.it_vendor_id}`);
