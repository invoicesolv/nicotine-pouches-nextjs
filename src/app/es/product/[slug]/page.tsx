...
async function getProduct(slug: string, packSize: string = '1pack', shippingFilter: string = 'fastest', sortFilter: string = 'low-high') {
  try {
    ...
    // Build select with columns that exist in es_vendor_products + es_vendors
    ...
    if (mappings && mappings.length > 0) {
      ...
      const { data: vpData, error: vpError } = await supabase()
        ...
        .in('es_vendor_id', mappings.map((m: any) => m.es_vendor_id))
        .limit(20);
    ...
    vendorProducts?.forEach((vp: any) => {
      const vendorKey = vp.es_vendor_id;
      const vendor = vp.es_vendors || {};
  ...
  try {
    ...
    if (brandProducts && brandProducts.length > 0) {
      ...
      vpMappings.forEach((m: any) => {
        ...
        vendorSets.get(m.product_id)!.add(m.es_vendor_id);
      });
      ...
      if (vpMappings.length > 0) {
        ...
        if (vpPrices) {
          ...
          vpMappings.forEach((m: any) => {
            nameVendorToProduct.set(`${m.vendor_product}__${m.es_vendor_id}`, m.product_id);
          });

          vpPrices.forEach((vp: any) => {
            const productId = nameVendorToProduct.get(`${vp.name}__${vp.es_vendor_id}`);
