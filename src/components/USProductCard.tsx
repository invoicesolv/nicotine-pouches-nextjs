...
const USProductCard = ({
  ...
  // Generate random price for US products
  const generatePrice = () => {
  ...
  const getStoreCount = (product: USProduct) => {
    return product.store_count || 0;
  };

  // Use actual watching count from database
  const getWatchingCount = (product: USProduct) => {
    return product.watching_count || 0;
  };
  ...
  const watching = getWatchingCount(product);
  ...
  return (
    <div className="swiper-slide" style={{ 
      ...
      <div className="product-card" 
        ...
        <Link href={`/us/product/${slug}`} className="product-link" style={{ 
          position: 'relative',
          zIndex: 1,
          textDecoration: 'none',
          color: 'inherit',
