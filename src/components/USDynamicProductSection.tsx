...
export default function USDynamicProductSection({
  title,
  section
  ...
  // Initial load only - fetch from us_products table
  useEffect(() => {
    const loadProducts = async () => {
      try {
        // Fetch more products to have variety, then shuffle and take subset
        const { data: usProducts, error } = await supabase()
          .from('us_products')
          .select('*')
          .not('image_url', 'is', null)
          .limit(100); // Fetch more to ensure good mix

        if (error) throw error;

        // Fetch real price alert tracking counts (by product_id)
        ...
        // Fetch real vendor counts from us_vendor_product_mapping
        const { data: vendorMappings } = await supabase()
          .from('us_vendor_product_mapping')
          .select('product_id, us_vendor_id');

        const vendorCounts = new Map<number, number>();
        vendorMappings?.forEach((mapping: any) => {
          if (mapping.product_id) {
            const count = vendorCounts.get(mapping.product_id) || 0;
            vendorCounts.set(mapping.product_id, count + 1);
          }
        });

        // Process products to match Product interface
        const processedProducts = (usProducts || []).map((product: any) => {
          const name = product.product_title || product.name || 'Unknown Product';
          return {
            id: product.id,
            name: name,
            brand: product.brand || 'Unknown',
            image_url: product.image_url,
            price: `$${(Math.random() * 3 + 4).toFixed(2)}`, // Random price $4-7
            store_count: vendorCounts.get(product.id) || 0, // Real vendor count from mappings
            tracking_count: trackingCounts.get(product.id) || 0, // Real tracking count by product_id
            trend_score: Math.random() * 100,
            strength_group: product.strength || 'Normal'
          };
        });

        // Use different seeds for each section to get different shuffles
        const sectionSeed = section === 'popular' ? 12345 : section === 'trending' ? 67890 : 24680;
        const shuffled = shuffleArray(processedProducts, sectionSeed);

        // Take first 15 products for each section (gives enough for scrolling)
        const sectionOffset = getSectionOffset();
        setProducts(shuffled.slice(sectionOffset, sectionOffset + 15));
      } catch (error) {
            ...
            return (
              ...
              >
                <div style={{
                  ...
                  <Link href={`/us/product/${slug}`} style={{
                    position: 'relative',
                    zIndex: 1,
                    textDecoration: 'none',
                    color: 'inherit',
