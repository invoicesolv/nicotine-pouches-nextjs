...
const SSRUSProductGridWithSidebar = async ({ brandFilter, vendorFilter, isUSRoute = true, currentPage = 1, filters }: SSRProductGridProps) => {
  ...
  try {
    ...
    if (vendorFilter) {
      ...
      let countQuery = supabase().from('us_products').select('*', { count: 'exact', head: true });
      let query = supabase().from('us_products').select('*');

      // Apply filters from URL params
      if (activeFilters.brand) {
        countQuery = countQuery.ilike('product_title', `${activeFilters.brand}%`);
        query = query.ilike('product_title', `${activeFilters.brand}%`);
      }
      if (activeFilters.flavour) {
        countQuery = countQuery.ilike('product_title', `%${activeFilters.flavour}%`);
        query = query.ilike('product_title', `%${activeFilters.flavour}%`);
      }
        ...
        <div className="products-mobile" style={{
          ...
          <div className="products-grid-mobile swiper-wrapper" style={{
            ...
            {products.length === 0 ? (
              ...
              products.map((product) => (
                <USProductCardWithDropdown key={product.id} product={product} />
              ))
            )}
          </div>
          ...
          {totalProducts > PRODUCTS_PER_PAGE && (
            <div style={{
              ...
              {currentPage > 1 && (
                <Link
                  href={`/us/compare?page=${currentPage - 1}`}
                  style={{
                    padding: '8px 16px',
                    borderRadius: '8px',
              ...
              {(() => {
                ...
                if (start > 1) {
                  pages.push(
                    <Link key={1} href="/us/compare?page=1" style={{
                      padding: '8px 12px',
                      borderRadius: '8px',
                      border: '1px solid #e5e7eb',
                      backgroundColor: '#fff',
                ...
                if (end < totalPages) {
                  ...
                  pages.push(
                    <Link key={totalPages} href={`/us/compare?page=${totalPages}`} style={{
                      padding: '8px 12px',
                      borderRadius: '8px',
                      border: '1px solid #e5e7eb',
                      backgroundColor: '#fff',
