...
interface USProductCardWithDropdownProps {
  product: Product;
}
...
let usPriceAlertsCache: { userId: string; productIds: Set<number>; timestamp: number } | null = null;
const CACHE_DURATION = 60000; // 1 minute cache

export default function USProductCardWithDropdown({ product }: USProductCardWithDropdownProps) {
  ...
  useEffect(() => {
    const fetchPriceAlerts = async () => {
      ...
      // Check cache first
      if (usPriceAlertsCache &&
          usPriceAlertsCache.userId === user.id &&
          Date.now() - usPriceAlertsCache.timestamp < CACHE_DURATION) {
        setHasAlert(usPriceAlertsCache.productIds.has(product.id));
        return;
      }
    ...
    const handleAlertUpdate = () => {
      usPriceAlertsCache = null; // Invalidate cache
      fetchPriceAlerts();
    };
